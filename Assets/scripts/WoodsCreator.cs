...
public class WoodsCreator : MonoBehaviour
{
    ...
    void Update()
    {
        if (timer > maxTime)
        {
            ...
            newWood.transform.position = transform.position + new Vector3(0, Random.Range(-hight, hight), 0);
        ...
        if (fruitFlag && timerFruit > 0.5f * maxTime && fruitIsAllowed)
        {
            ...
            newFruit.transform.position = transform.position + new Vector3(0, Random.Range(-hight * 2, hight * 2), 0);
